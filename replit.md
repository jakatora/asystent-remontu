...
- **Core Logic**:
    ...
    - `features/house-build/`: "House Build Assistant" module — 13 enriched build stages (land purchase through final inspections, including insulation-energy stage), each with whyItMatters, investorNotes, completionCriteria, beforeNextStage checks, professionalLabels (with required/optional distinction), source metadata, stage-gate support. 12 professional roles, 8 construction warnings, GLOBAL_BUILD_NOTES for recurring project/system documentation priority reminders. Energy planning screen with U-value and EP target fields. Prompt 4: stage dependency logic (`dependencies.ts`), 9 default milestones (`milestones.ts`), stage-to-contractor specialty mapping (`contractor-mapping.ts`). Prompt 5: 8 official form definitions (`official-forms.ts`), 18 question templates by stage/role (`question-templates.ts`), 10 completion package items (`completion-package.ts`), 10 decision templates (`decision-defaults.ts`).
    - `types/house-build.ts`: All house-build domain types — `HouseBuildProject`, `LandContext`, `PlanningContext`, `BuildStage`, `BuildChecklistItem`, `DocumentRequirement`, `OfficialProcessStep`, `UtilityRequirement`, `ProfessionalRoleRequirement`, `ConstructionRiskNotice`, `BuildProjectTimelineItem`, `SourceMetadata`, plus formal path types, plus Prompt 4 timeline/budget types: `TimelineStageRecord`, `StageBudgetItem`, `StageBudgetNote`, `BuildTimelineMilestone`, `TimelineNoteRecord`, `StageProfessionalPlan`, `EnergyStrategyRecord`, `HouseBuildBudget`, `StageBudgetSummary`, plus Prompt 5 types: `OfficialFormRecord`, `InvestorDocRecord`, `BuildDecisionRecord`, `BuildQuestionRecord`, `CompletionPackageItem`, `DocDashboardSummary`, plus enums: `TimelineStageStatus`, `TimelineNoteType`, `StageCostCategory`, `BudgetCompletenessState`, `ProfessionalNeedState`, `StageManagementMode`, `ApplicabilityState`, `OfficialFormStatus`, `InvestorDocGroup`, `InvestorDocStatus`, `DecisionCategory`, `DecisionStatus`, `QuestionPriority`.
    ...
    - `db/migrations/007_timeline_budget.ts`: 7 new SQLite tables: `build_timeline_stages`, `build_budget_items`, `build_budget_notes`, `build_milestones`, `build_timeline_notes`, `build_professional_plans`, `build_energy_strategy` with indexes.
    - `db/migrations/008_investor_docs.ts`: 5 new SQLite tables: `build_official_forms`, `build_investor_docs`, `build_decisions`, `build_questions`, `build_completion_items` with UNIQUE indexes for upsert operations.
    - `db/repositories/house-build.repo.ts`: Full CRUD for house-build projects, checklists, documents, and utilities. deleteProject cascades to all Prompt 4 + Prompt 5 tables (16 tables total).
    - `db/repositories/timeline-budget.repo.ts`: Full CRUD for timeline stages (upsert), budget items (add/update/delete), milestones (upsert), timeline notes, professional plans (upsert), energy strategy (upsert), budget notes (get/add/delete), plus deleteProjectData cascade.
    - `db/repositories/investor-docs.repo.ts`: Full CRUD for official forms (upsert), investor documents (add/update/delete), decisions (add/update/delete), questions (add/update/delete), completion items (upsert), plus deleteProjectData cascade.
    - `app/house-build/`: 25 screens total — module home, create wizard, project detail (with 15 quick-action tiles incl. Harmonogram/Budzet/Milestones/Centrum dok./Decyzje/Pytania), stage detail, energy-planning, documents overview, professionals guide, utilities tracker, PLUS 7 formal screens, PLUS 4 Prompt 4 screens: timeline, budget, milestones, stage-plan, PLUS 6 Prompt 5 screens: doc-dashboard (summary with routing to sub-screens), official-forms (8 PB form tracker with status cycle and applicability), decisions (10 seeded decision templates with options/selected/reasoning editing), questions (question manager with role filtering, answer recording, and 18 seeded templates), completion-package (10 completion items with progress bar and status cycle), investor-docs (grouped document manager with add/delete/status).
- **Project Structure**:
