...
- **Core Logic**:
    ...
    - `app/house-build/`: 15 screens total — module home, create wizard, project detail (with 10 quick-action tiles), stage detail (enhanced with whyItMatters, investor notes, professionals, completion criteria, before-next-stage blocks, global notes toggle, source footer), energy-planning (U-value/EP target planning sheet), documents overview, professionals guide, utilities tracker, PLUS 7 formal screens: formal-path (3-step wizard), formal-result (path assessment with pros/cons/cautions), formal-documents (official checklists with progress), before-works (pre-construction checklist by category), start-works (notification requirements), edb (Electronic Construction Log guide), completion (notice vs permit paths with closing checklist).
- **Project Structure**:
