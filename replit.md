...
- **Core Logic**:
    ...
    - `db/repositories/house-build.repo.ts`: Full CRUD for house-build projects, checklists, documents, and utilities. deleteProject also cleans all 7 Prompt 4 tables.
    - `db/repositories/timeline-budget.repo.ts`: Full CRUD for timeline stages (upsert), budget items (add/update/delete), milestones (upsert), timeline notes, professional plans (upsert), energy strategy (upsert), plus deleteProjectData cascade.
    - `app/house-build/`: 19 screens total — module home, create wizard, project detail (with 13 quick-action tiles incl. Harmonogram/Budzet/Milestones), stage detail, energy-planning, documents overview, professionals guide, utilities tracker, PLUS 7 formal screens, PLUS 4 Prompt 4 screens: timeline (stage ordering with progress, milestone chips, blocked/waiting states, risk notes), budget (stage-by-stage cost blocks with category split, add/delete items, completeness badges), milestones (9 default milestones with status toggle and notes), stage-plan (stage planning detail with status/management/custom name/weeks/notes/dependencies/professional need states/contractor search placeholder/risk notes).
- **Project Structure**:
