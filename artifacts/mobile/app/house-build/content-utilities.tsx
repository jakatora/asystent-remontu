...
export default function ContentUtilitiesScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const data = await houseBuildContentAdminRepo.getContentItems('utility-guidance');
    setItems(data);
    setLoading(false);
  ...
  const startEdit = (item: AdminContentItem) => { setEditingId(item.id); setEditTitle(item.title); setEditSummary(item.summary); setEditBody(item.body); };
  const saveEdit = async (item: AdminContentItem) => { await houseBuildContentAdminRepo.upsertContentItem({ ...item, title: editTitle, summary: editSummary, body: editBody }); setEditingId(null); load(); };
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: Colors.background }} contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 32 }}>
      <TextInput style={{ backgroundColor: '#FFFFFF', borderRadius: 8, padding: 10, borderWidth: 1, borderColor: '#E2E8F0', fontSize: 13, marginBottom: 12 }} placeholder="Szukaj przylacza..." value={search} onChangeText={setSearch} placeholderTextColor={Colors.textMuted} />
      ...
      {filtered.map((item) => (
        <View key={item.id} style={{ backgroundColor: '#FFFFFF', borderRadius: 10, padding: 12, borderWidth: 1, borderColor: '#E2E8F0', marginBottom: 8 }}>
          {editingId === item.id ? (
            <View style={{ gap: 8 }}>
              <TextInput style={{ backgroundColor: '#F8FAFC', borderRadius: 6, padding: 8, fontSize: 13, borderWidth: 1, borderColor: '#E2E8F0' }} value={editTitle} onChangeText={setEditTitle} placeholderTextColor={Colors.textMuted} />
              <TextInput style={{ backgroundColor: '#F8FAFC', borderRadius: 6, padding: 8, fontSize: 12, borderWidth: 1, borderColor: '#E2E8F0' }} value={editSummary} onChangeText={setEditSummary} multiline placeholderTextColor={Colors.textMuted} />
              <TextInput style={{ backgroundColor: '#F8FAFC', borderRadius: 6, padding: 8, fontSize: 12, borderWidth: 1, borderColor: '#E2E8F0', minHeight: 60 }} value={editBody} onChangeText={setEditBody} multiline placeholderTextColor={Colors.textMuted} />
