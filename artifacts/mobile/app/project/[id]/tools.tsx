...
export default function ProjectToolsScreen() {
  ...
  if (!project || !job) {
    return (
      <View className="flex-1 items-center justify-center bg-bg">
        <Txt w="medium" className="text-base text-slate">{t('project.tools.notFound')}</Txt>
      </View>
    );
  }
  ...
  return (
    <>
      <Stack.Screen
        options={{
          title: t('project.tools.title'),
          headerBackTitle: t('project.tools.headerBack'),
      ...
      >
        ...
        {tools.length > 0 && (
          <View className="bg-surface rounded-2xl border border-stroke p-4 mb-5">
            <View className="flex-row justify-between items-center mb-2">
              <Txt w="semibold" className="text-sm text-ink">{t('project.tools.prepared')}</Txt>
            ...
            {checkedCount === tools.length && tools.length > 0 && (
              <View className="flex-row items-center gap-2 mt-2">
                ...
                <Txt w="medium" className="text-xs text-success">{t('project.tools.allPrepared')}</Txt>
              </View>
            )}
          </View>
        )}
        ...
        {required.length > 0 && (
          <View className="mb-5">
            <View className="flex-row items-center gap-2 mb-3">
              ...
              <Txt w="bold" className="text-base text-ink">{t('project.tools.requiredTitle')}</Txt>
        ...
        {optional.length > 0 && (
          <View className="mb-5">
            <View className="flex-row items-center gap-2 mb-3">
              ...
              <Txt w="bold" className="text-base text-ink">{t('project.tools.optionalTitle')}</Txt>
                  ...
                  <View className="flex-1">
                    ...
                    {tool.rentable && (
                      <View className="self-start mt-1 px-2 py-0.5 rounded-lg bg-info-bg">
                        <Txt w="medium" className="text-[10px] text-info">{t('project.tools.rentBadge')}</Txt>
                      </View>
                    )}
                  </View>
                  <View className={`w-6 h-6 rounded-full border-2 items-center justify-center ${checked ? 'bg-success border-success' : 'border-stroke'}`}>
        ...
        {tools.length === 0 && (
          <View className="items-center py-8 gap-3">
            ...
            <Txt w="bold" className="text-lg text-ink">{t('project.tools.emptyTitle')}</Txt>
            <Txt className="text-sm text-slate text-center" style={{ maxWidth: 260 }}>
              {t('project.tools.emptyBody')}
            </Txt>
          </View>
        )}
